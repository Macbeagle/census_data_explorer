...
download_census_data <- function(c_year, c_pack, c_geo, c_area, dest_path, extract_path, progress = NULL) {
  ...
  print(url)
  if (!is.null(progress)) progress$set(message = "Starting download...", value = 0.1)
  files <- list.files(extract_path, full.names = TRUE)
  tryCatch({
    download.file(url, destfile = file.path(dest_path, file_name), mode = "wb")
    if (!is.null(progress)) progress$set(message = "Unzipping file...", value = 0.5)
    unzip(file.path(dest_path, file_name), exdir = extract_path)
    if (!is.null(progress)) progress$set(message = "Cleaning up...", value = 0.9)
    file.remove(file.path(dest_path, file_name))
    if (!is.null(progress)) progress$set(message = "Download successful!", value = 1)
    return("Download successful!")
  }, error = function(e) {
    if (!is.null(progress)) progress$set(message = "Failed to download", value = 1)
